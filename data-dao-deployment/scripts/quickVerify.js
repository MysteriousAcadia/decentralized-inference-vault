...
const VERIFICATION_DATA = {
  ...
  sepolia: {
    CommunityAccessDAOFactory: {
      ...
      contract: "contracts/CommunityAccessDAOFactory.sol:CommunityAccessDAOFactory",
      args: [
...
async function quickVerify() {
  ...
  if (!networkData) {
    console.log(`❌ No verification data available for network: ${hre.network.name}`);
    console.log(`Available networks: ${Object.keys(VERIFICATION_DATA).join(", ")}`);
    return;
  }
...
async function verifySpecificContract(contractName, networkData) {
  ...
  if (!contractData) {
    console.log(`❌ Contract '${contractName}' not found for ${hre.network.name}`);
...
async function interactiveVerification(networkData) {
  ...
  console.log(`\n🎯 Verification completed: ${successCount}/${Object.keys(networkData).length} successful`);
}
...
async function performVerification(contractData) {
  ...
  try {
    ...
    console.log(`🔍 View on explorer: ${getExplorerLink(contractData.address)}`);
    return true;
    ...
    if (error.message.toLowerCase().includes("already verified")) {
      ...
      if (error.message.includes("API Key")) {
        console.log("💡 Tip: Make sure you have the correct API key set in your .env file");
      } else if (error.message.includes("constructor")) {
        console.log("💡 Tip: Constructor arguments might be incorrect. Double-check the deployment transaction.");
      } else if (error.message.includes("bytecode")) {
        console.log("💡 Tip: Make sure the contract source code matches exactly what was deployed.");
      }
...
function checkApiKey() {
  ...
  switch (hre.network.name.toLowerCase()) {
    ...
    case 'base':
      ...
      break;
    default:
      console.log(`⚠️  Unknown network: ${hre.network.name}. Proceeding anyway...`);
      return true;
  }
  
  if (!requiredKey) {
    console.log(`❌ Missing ${keyName} in environment variables`);
    console.log(`💡 Get your API key from the appropriate block explorer and add it to your .env file`);
