...
async function main() {
  ...
  try {
    ...
    console.log(`Approving ${hre.ethers.formatEther(purchaseAmount)} tokens...`);
    ...
    console.log(`🔐 Access time: ${accessBalance.toString()} seconds (${accessBalance.toString() / 3600} hours)`);
  ...
  const fs = require('fs');
  const filename = `working-dao-deployment-${hre.network.name}-${Date.now()}.json`;
  ...
  console.log(`🔗 DAO on Explorer: https://sepolia.etherscan.io/address/${daoAddress}`);
