...
const CONTRACT_ADDRESSES = {
  sepolia: {
    communityDAOFactory:
      process.env.COMMUNITY_DAO_FACTORY_ADDRESS ||
      "0xEB37A065E20D0BB04b161B1d2985065Fb242866a",
    testToken:
      process.env.PAYMENT_TOKEN_ADDRESS ||
      "0xb4452088fAa8920b026Cd52Bb7eca958f984B1D4",
    dataCoinFactory:
      process.env.DATACOIN_FACTORY_ADDRESS ||
      "0xC7Bc3432B0CcfeFb4237172340Cd8935f95f2990",
  },
  amoy: {
...
async function main() {
  ...
  console.log(
    `🏭 CommunityDAOFactory: ${addresses.communityDAOFactory || "Not set"}`
  );
  ...
  if (addresses.testToken) {
    console.log(
      `🪙 TestToken: ${getExplorerLink(addresses.testToken, networkName)}`
    );
  }
  if (addresses.communityDAOFactory) {
    console.log(
      `🏭 CommunityDAOFactory: ${getExplorerLink(
        addresses.communityDAOFactory,
        networkName
      )}`
    );
  ...
  if (addresses.dataCoinFactory) {
    console.log(
      `🏗️  DataCoinFactory: ${getExplorerLink(
        addresses.dataCoinFactory,
        networkName
      )}`
    );
  }
...
async function verifyCommunityDAOFactory(
  address,
  ...
  return await verifyContract(
    address,
    [defaultTreasury, dataCoinFactory],
    "CommunityAccessDAOFactory"
  );
}
