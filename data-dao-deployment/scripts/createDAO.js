...
async function main() {
  ...
  if (!factoryAddress || !paymentTokenAddress || !dataCoinFactoryAddress) {
    ...
    console.log(
      `   COMMUNITY_DAO_FACTORY_ADDRESS: ${factoryAddress || "MISSING"}`
    );
    console.log(
      `   PAYMENT_TOKEN_ADDRESS: ${paymentTokenAddress || "MISSING"}`
    );
    console.log(
      `   DATACOIN_FACTORY_ADDRESS: ${dataCoinFactoryAddress || "MISSING"}`
    );
    ...
    console.log(
      `✅ Using factory: ${
        factoryAddress || chainConfig.communityDAOFactoryAddress
      }`
    );
  }
  ...
  console.log(
    `   Creator Allocation: ${DAO_CONFIG.dataCoin.creatorAllocationBps / 100}%`
  );
  console.log(
    `   Contributors Allocation: ${
      DAO_CONFIG.dataCoin.contributorsAllocationBps / 100
    }%`
  );
  console.log(
    `   Liquidity Allocation: ${
      DAO_CONFIG.dataCoin.liquidityAllocationBps / 100
    }%`
  );
  console.log(`\n🔐 Access Settings:`);
  console.log(
    `   Seconds per Token: ${DAO_CONFIG.access.secondsPerToken} (${
      DAO_CONFIG.access.secondsPerToken / 3600
    } hours)`
  );
  console.log(
    `   Reward Rate: ${DAO_CONFIG.access.rewardRate} DataCoin per payment token`
  );
  ...
  console.log(
    `   Token Balance: ${hre.ethers.formatEther(
      tokenBalance
    )} ${await paymentToken.symbol()}`
  );

  if (ethBalance < hre.ethers.parseEther("0.01")) {
    console.log(
      "⚠️  Warning: Low ETH balance. You may need more ETH for gas fees."
    );
  }
  ...
  let dataCoinFactory;
  try {
    dataCoinFactory = await hre.ethers.getContractAt(
      "IDataCoinFactory",
    ...
    try {
      const minLockAmount = await dataCoinFactory.getMinLockAmount(
        DAO_CONFIG.dataCoin.lockToken
      );
      console.log(
        `✅ Real DataCoin Factory detected. Min lock amount: ${minLockAmount}`
      );
    } catch (e) {
      console.log(
        "⚠️  Mock DataCoin Factory detected or lock token not supported"
      );
    ...
    console.log(
      "⚠️  Could not connect to DataCoin Factory, using minimal parameters"
    );
  }
  ...
  const dcParams = {
    ...
    salt: hre.ethers.keccak256(
      hre.ethers.toUtf8Bytes(
        `${Date.now()}-${signer.address}-${DAO_CONFIG.dataCoin.name}`
      )
    ),
  };
  ...
  try {
    ...
    try {
      const gasEstimate = await factory.createCommunityAccessDAO.estimateGas(
        dcParams,
        apParams
      );
      ...
      console.log(
        "❌ Gas estimation failed. This usually indicates the transaction will revert."
      );
      ...
      if (gasError.message.includes("revert")) {
        throw new Error(
          "Transaction would revert. Check contract parameters and factory implementation."
        );
      }
      throw gasError;
    }
    ...
    const daoDeployedEvent = logs.find(
      (log) => log.name === "CommunityDAODeployed"
    );
    ...
    if (daoDeployedEvent) {
      ...
      console.log(
        `🪙 DataCoin Contract: ${explorerBaseUrl}/address/${dataCoin}`
      );
      ...
      const fs = require("fs");
      const deploymentFileName = `dao-deployment-${
        hre.network.name
      }-${Date.now()}.json`;
      fs.writeFileSync(
        deploymentFileName,
      ...
      console.log(
        `   TestToken.mintTokens("${signer.address}", ethers.parseEther("1000"))`
      );
      console.log("2. 🔐 Purchase access:");
      console.log(
        `   DAO.buyAccess(ethers.parseEther("10"), "${signer.address}")`
      );
      ...
      console.log(
        "⚠️  Could not parse deployment event. Transaction succeeded but details unavailable."
      );
    }
  } catch (error) {
...
async function testDAOFunctionality(
  ...
  try {
    ...
    const dataCoin = await hre.ethers.getContractAt(
      "TestToken",
      dataCoinAddress,
      signer
    ); // Assuming DataCoin has similar interface
    const paymentToken = await hre.ethers.getContractAt(
      "TestToken",
      paymentTokenAddress,
      signer
    ...
    console.log(
      `   ✅ Approved ${hre.ethers.formatEther(purchaseAmount)} tokens`
    );
    ...
    console.log(
      `   DataCoin Balance: ${hre.ethers.formatEther(dataCoinBalance)}`
    );
