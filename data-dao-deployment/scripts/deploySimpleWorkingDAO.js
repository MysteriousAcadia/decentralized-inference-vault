  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(`   🔐 New access balance: ${newAccess.toString()} seconds`);
        
      } catch (buyError) {
        console.log("   ❌ Access purchase failed:", buyError.message);
        if (buyError.message.includes("INTERFACE_NOT_SUPPORTED")) {
          console.log("   💡 This is expected - TestToken doesn't implement IDataCoin interface");
          console.log("   💡 The DAO expects a proper DataCoin with mint() function");
        }
      }
    ...
    const fs = require('fs');
    const filename = `simple-dao-deployment-${hre.network.name}-${Date.now()}.json`;
    ...
    console.log(`🔗 DAO Explorer: ${getExplorerUrl(hre.network.name)}/address/${daoAddress}`);
    console.log(`💾 Details saved to: ${filename}`);
    
    console.log("\n🎯 NEXT STEPS:");
    console.log("1. The DAO is deployed but may need proper DataCoin interface");
