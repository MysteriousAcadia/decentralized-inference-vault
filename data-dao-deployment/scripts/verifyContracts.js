...
async function verifyAllContracts() {
  ...
  if (!networkContracts) {
    console.log(`❌ No deployed contracts found for network: ${hre.network.name}`);
    console.log("Available networks:", Object.keys(DEPLOYED_CONTRACTS).join(", "));
    return;
  }
  ...
  if (!requiredApiKey) {
    console.log(`❌ Missing API key for ${hre.network.name}. Please set the appropriate environment variable.`);
    return;
  }
  ...
  for (const [contractName, contractInfo] of Object.entries(networkContracts)) {
    ...
    console.log(`🔧 Constructor Args: ${JSON.stringify(contractInfo.constructorArgs, null, 2)}`);
    ...
    if (Object.keys(networkContracts).indexOf(contractName) < Object.keys(networkContracts).length - 1) {
...
async function verifySpecificContract(contractName, address, ...constructorArgs) {
  ...
  if (success) {
    ...
    console.log(`🔍 View on explorer: ${getExplorerLink(address, hre.network.name)}`);
  } else {
...
console.log("1. Verify all contracts: npx hardhat run scripts/verifyContracts.js --network sepolia");
console.log("2. Verify specific contract: npx hardhat run scripts/verifyContracts.js --network sepolia -- ContractName 0x... arg1 arg2");
console.log("");
