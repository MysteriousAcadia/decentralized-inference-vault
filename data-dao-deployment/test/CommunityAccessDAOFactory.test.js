...
describe("CommunityAccessDAOFactory", function () {
  async function deployFixture() {
    ...
    const paymentToken = await ERC20Mock.deploy("PayToken", "PAY", deployer.address, ethers.parseUnits("100000", 18));
    ...
    const CommunityAccessDAOFactory = await ethers.getContractFactory("CommunityAccessDAOFactory");
    const factory = await CommunityAccessDAOFactory.deploy(ethers.ZeroAddress, dataCoinFactory.target);
    ...
    return { deployer, user, recipient, paymentToken, dataCoinFactory, factory, dcParams, apParams };
  }

  it("deploys DAO and DataCoin and mints rewards on access purchase", async function () {
    const { factory, dcParams, apParams, deployer, user, recipient, paymentToken } = await deployFixture();

    // fund user with payment tokens
    await paymentToken.transfer(user.address, ethers.parseUnits("1000", 18));

    // create DAO
    const tx = await factory.connect(user).createCommunityAccessDAO(dcParams, apParams);
    ...
    await paymentToken.connect(user).approve(daoAddress, ethers.parseUnits("10", 18));
    const beforeExpiry = await dao.getExpiry(recipient.address);
    expect(beforeExpiry).to.equal(0);

    await expect(dao.connect(user).buyAccess(ethers.parseUnits("10", 18), recipient.address))
