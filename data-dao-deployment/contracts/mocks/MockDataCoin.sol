...
contract MockDataCoin is IDataCoin {
    ...
    function grantRole(bytes32 role, address account) external override {
        require(hasRole(_DEFAULT_ADMIN_ROLE, msg.sender), "NO_ADMIN");
    ...
    function renounceRole(bytes32 role, address account) external override {
        require(account == msg.sender, "NOT_SELF");
        _roles[role][account] = false;
    }
    ...
    function MINTER_ROLE() external pure override returns (bytes32) {
    ...
    function getVestingInfo()
        ...
        pure
        override
        returns (VestingConfig memory)
    {
        return VestingConfig(0, 0, 0, 0, address(0));
    }
    ...
    function getRemainingContributorsalloc()
        external
        pure
        override
        returns (uint256)
