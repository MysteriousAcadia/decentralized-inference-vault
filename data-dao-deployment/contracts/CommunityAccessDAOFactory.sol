...
contract CommunityAccessDAOFactory {
    ...
    // ============ Events ============
    event CommunityDAODeployed(address indexed owner, address daoAddress, address dataCoin, address paymentToken, uint256 secondsPerToken, uint256 rewardRate, address treasury, uint256 index);
    event TreasuryDefaultUpdated(address indexed oldTreasury, address indexed newTreasury);
    ...
    function createCommunityAccessDAO(
        ...
        emit CommunityDAODeployed(msg.sender, daoAddress, dataCoinAddr, ap.paymentToken, ap.secondsPerToken, ap.rewardRate, _treasury, allDAOs.length - 1);
    }

    // Internal helper to isolate stack depth
    function _createDataCoin(DataCoinParams memory dc) internal returns (address coinAddr) {
    ...
    function getDAOsByOwner(address owner_) external view returns (address[] memory) {
