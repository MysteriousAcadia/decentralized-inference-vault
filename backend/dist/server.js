...
app.use(cors({
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'x-wallet-address', 'x-wallet-signature', 'x-wallet-message', 'x-wallet-timestamp']
}));
// Rate limiting
...
app.listen(PORT, () => {
    ...
    // Log configuration
    console.log('\n📋 Configuration:');
    console.log(`- Factory Address: ${process.env.COMMUNITY_ACCESS_DAO_FACTORY_ADDRESS || 'Default Sepolia'}`);
    console.log(`- RPC URL: ${process.env.ETHEREUM_RPC_URL ? 'Configured' : 'Using default'}`);
    console.log(`- Public Key: ${process.env.PUBLIC_KEY ? 'Configured' : 'Missing'}`);
