...
export class LighthouseService {
     ...
     */
    async signAuthMessage(publicKey, privateKey) {
        try {
     ...
     */
    async downloadFile(cid, customFileName) {
        try {
            if (!cid) {
                return {
                    success: false,
            ...
            try {
                fileEncryptionKey = await lighthouse.fetchEncryptionKey(cid, this.publicKey, signedMessage);
            }
            catch (error) {
                return {
                    success: false,
     ...
     */
    async canAccessFile(cid) {
        try {
            const signedMessage = await this.signAuthMessage(this.publicKey, this.privateKey);
            const fileEncryptionKey = await lighthouse.fetchEncryptionKey(cid, this.publicKey, signedMessage);
