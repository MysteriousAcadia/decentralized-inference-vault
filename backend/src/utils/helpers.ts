...
export function validateEnvironment(): { isValid: boolean; errors: string[] } {
  ...
  const requiredEnvVars = ["PRIVATE_KEY", "PUBLIC_KEY", "ETHEREUM_RPC_URL"];
  ...
  if (
    ...
    !isValidEthereumAddress(process.env.PUBLIC_KEY)
  ) {
  ...
  if (
    ...
    !/^0x[a-fA-F0-9]{64}$/.test(process.env.PRIVATE_KEY)
  ) {
    errors.push(
