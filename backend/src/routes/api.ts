...
import {
  LighthouseService,
  isValidCID,
} from "../services/LighthouseService.js";
import {
  authenticateWallet,
  authenticateWalletHeader,
  optionalAuth,
  generateAuthMessage,
 ...
 */
router.post(
  "/access/dao/:daoAddress",
  [
    param("daoAddress")
      .isEthereumAddress()
      .withMessage("Valid DAO address is required"),
  ],
  handleValidationErrors,
  authenticateWallet,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const result = await accessService.checkDAOAccess(
        daoAddress,
        userAddress
      );
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const downloadResult = await lighthouseService.downloadFile(
        cid,
        fileName
      );
